...
def main():
    ...
    sources = ['Data/Twitter/data_tensor_top1000.pickle']
    ...
    for i in np.arange(2):
        ...
        train_sequential_fresh = False  ### need to set if_sample_from_tweets_mode = False
        display_sequential_dictionary = False
        ...
        if display_dictionary:
            #path = 'Tweets_dictionary/dict_learned_CPDL_' + save_file_name + '.npy'
            path = "dict_learned_CPDL_top1000_ntopics_20_iter_50_sparsity_1_batchsize_100_ref_history_as_1.npy"
            W = np.load(path, allow_pickle=True).item()
            """
            reconstructor.display_dictionary_CP(W,
                                                ...
                                                if_plot=True)
            """
            reconstructor.display_heatmap_topics(W)
